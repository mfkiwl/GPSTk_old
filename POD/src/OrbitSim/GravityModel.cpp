...
namespace POD
{
       ...
       */
   GravityModel::GravityModel(int n, int m)
         : desiredDegree(n),
           desiredOrder(m),
   ...
   void loadModel(const std::string &path)
   {
       ...
       */
   void GravityModel::computeVW(Vector<double> r, Matrix<double> E)
   {   
       ...
       */
   Vector<double> GravityModel::gravity(Vector<double> r, Matrix<double> E)
   {
         // dimension should be checked here
         // I'll do it latter...
      if((r.size()!=3) || (E.rows()!=3) || (E.cols()!=3))
       ...
       * @param E ECI to ECEF transformation matrix.
       */
   Matrix<double> GravityModel::gravityGradient(gpstk::Vector<double> r, gpstk::Matrix<double> E)
   {
         // dimension should be checked here
         // I'll do it latter...
      if((r.size()!=3) || (E.rows()!=3) || (E.cols()!=3))
       ...
       */
   void GravityModel::doCompute(Epoch time, EarthBody& rb, Spacecraft& sc)
   {

      //Matrix<double> C2T = ReferenceFrames::J2kToECEFMatrix(time);
      Matrix<double> C2T = OrbitSim::erp.getJ2k2ECEF(time);
   ...
   void GravityModel::correctCSTides(Epoch t,bool solidFlag,bool oceanFlag,bool poleFlag)
   {
   ...
   double GravityModel::normFactor(int n, int m) 
   {
   ...
   //
   void GravityModel::loadModel(const std::string &path)
   {
       ...
       int isNormalize(0);
       int nMax(0), mMax(0);
       inpStream >> gmData.refDistance >> gmData.GM >> factor>> gmData.maxDegree >> gmData.maxOrder >> isNormalize;
           
       if(gmData.maxOrder < desiredOrder || gmData.maxDegree < desiredDegree)
       {
           Exception e("desired gravity field degree(order) is lover than maximum degree(order) avalable in "+ path+ " gravity model file.");
           GPSTK_THROW(e);
       }

       Matrix<double> & CS = (isNormalize) ? gmData.normalizedCS : gmData.normalizedCS;

       CS.resize(desiredOrder, desiredDegree);
       string buff;
       while (!inpStream.eof())
       {
           int n(0), m(0);
           ...
           CS[n][m] = cnm;
           CS[m - 1][n] = (m == 0) ? 0.0 : snm;
           if (n == desiredDegree && m == desiredOrder) break;
           
       }
   }
   //
   void GravityModel::test()
   {
